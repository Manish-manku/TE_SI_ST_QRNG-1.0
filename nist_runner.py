...
class _NIST:
    ...
    def maurers_universal(bits: np.ndarray,
        ...
        T: Dict[int, int] = {}
        for i in range(Q):
            block = int("".join(str(b) for b in bits[i*L:(i+1)*L]), 2)
            T[block] = i + 1
        fn = 0.0
        for i in range(Q, Q + K):
            block = int("".join(str(b) for b in bits[i*L:(i+1)*L]), 2)
            dist  = i + 1 - T.get(block, 0)
            if dist > 0:
                fn += math.log2(dist)
            T[block] = i + 1
        fn /= K
    ...
    def serial(bits: np.ndarray, m: int = 16) -> Tuple[Optional[float], str]:
        ...
        d2 = psi_sq(bits, m) - psi_sq(bits, m - 1)
        d2a = psi_sq(bits, m) - 2 * psi_sq(bits, m - 1) + psi_sq(bits, m - 2)
...
if __name__ == "__main__":
    print("NIST SP 800-22 Runner — self-test")
    print("=" * 60)
