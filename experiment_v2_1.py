...
try:
    from experiment_6_nist_validation import (
        NISTExperimentRunner,
        nist_summary_for_experiment_2,
        _worker_post_extraction,
    )
    from nist_runner import NISTTestRunner
...
class ExperimentRunner:
    ...
    def experiment_2_entropy_certification(self, n_bits: int = 3500000):
        ...
        # NIST SP 800-22 summary (supplement to Experiment 2)
        if _NIST_AVAILABLE:
            print("  [Exp 2 supplement] Running NIST SP 800-22 summary for all scenarios...")
            scenarios_obj  = create_test_scenarios()
            task_args_nist = [(name, params, min(n_bits, 1_000_000))
                              for name, params in scenarios_obj.items()]
            nist_post = {}
            with ProcessPoolExecutor(max_workers=self.max_workers) as exe:
                futures_nist = {exe.submit(_worker_post_extraction, a): a[0]
                                for a in task_args_nist}
                for fut in as_completed(futures_nist):
                    sc_name, sc_result = fut.result()
                    nist_post[sc_name] = sc_result
                    print(f"    NIST [{sc_name}]  pass_rate={sc_result["pass_rate"]:.1%}")
            nist_summary_for_experiment_2(nist_post, self.output_dir)
            with open(self.output_dir / "data" / "experiment_2_nist_summary.json", "w") as f:
                import json as _json
                _json.dump(nist_post, f, indent=2)
