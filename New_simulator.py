...
SourceParameters = (
    ...
    AttackedParams | PhotonCountingParams | PhaseNoiseParams
)
...
class QuantumSourceSimulator:
    ...
    def get_signal_stats(self) -> tuple[float, float]:
        ...
        if st == SourceType.PHASE_NOISE:
            # measured quadrature: randn*(1 + noise_level) → N(0, 1+noise_level)
            return 0.0, float(1.0 + p.noise_level)
    ...
    def _generate_attacked(self, n_bits: int) -> GeneratedBlock:
        """
        Adversarial attack: random bit-forcing + periodic pattern injection.

        The periodic pattern is now also encoded into raw_signal so that
        energy_constraint_test() can detect the anomalous periodicity.
        Previously raw_signal was flat randn + constant, which was invisible
        to spectral/energy tests.  Now the forcing positions produce a signal
        spike of +1.0 (above the attack_strength baseline) so the std of
        the signal exceeds the expected value — detectable by the test.
        """
        p = self.params
...
def create_test_scenarios() -> dict[str, SourceParameters]:
    """
    Return 12 pre-built SourceParameter objects for standard TE-SI-QRNG evaluation.

    Covers: ideal, biased, drifting, correlated, attacked,
